...
function MostTokensConversationsTable({ conversationsDataSource, timeRange, filters }: MostTokensTableProps) {
  ...
  const title = 'Highest token usage conversations';
  const seeMoreHref = buildTokensSeeMoreUrl(timeRange, filters);
      ...
      <table className={styles.table}>
        <thead>
          <tr className={styles.headerRow}>
            ...
            <th className={styles.headerCell}>Total tokens</th>
        ...
        <tbody>
          {conversations.map((conversation) => (
            ...
            >
              ...
              <td className={styles.tableCell}>{formatTokenCount(getConversationTotalTokens(conversation))}</td>
